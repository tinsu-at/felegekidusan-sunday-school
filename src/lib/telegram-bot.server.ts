...
const helpKeyboard = (lang: Lang, buttons: { text: string; url: string }[] = []) => ({
  inline_keyboard: [
...
async function notifyAdmins(lines: string[]) {
  ...
  for (const chatId of targets) {
    try { await telegram("sendMessage", { chat_id: chatId, text }); }
    catch { console.error("Admin notification could not be delivered"); }
  }
}
...
function validateAmharicName(value: string, exactWords: number | null, lang: Lang): string | null {
  const name = value.trim().replace(/\s+/g, " ");
  if (!name) return null;
  const words = name.split(" ");
  if (exactWords !== null && words.length !== exactWords) return null;
  // No word-count restriction when exactWords is null (Q2, Q5 and Q7).
  if (exactWords === null && (words.length < 1 || words.length > 100)) return null;
...
export async function handleTelegramUpdate(update: TelegramUpdate): Promise<void> {
  ...
  const saveSession = async (nextStep: Step | "idle", nextAnswers: Answers) => {
    await supabaseAdmin.from("registration_sessions").upsert({ telegram_user_id: userId, telegram_chat_id: chatId, telegram_username: username, step: nextStep, answers: nextAnswers }, { onConflict: "telegram_user_id" });
  };
  ...
  const goToConfirm = async (nextAnswers: Answers) => {
    ...
    if (!regId) { const { data } = await supabaseAdmin.rpc("reserve_registration_id"); regId = (data as string | null) ?? undefined; }
  ...
  const askNext = async (nextStep: Step, nextAnswers: Answers) => {
    if (nextStep === "confirm") { await goToConfirm(nextAnswers); return; }
    await saveSession(nextStep, nextAnswers);
    if (nextStep === "gender") await sendMessage(chatId, T[lang].questions.gender, genderKeyboard(lang));
    else await sendMessage(chatId, T[lang].questions[nextStep]);
  };
  ...
  if (cb?.data === "gender_male" || cb?.data === "gender_female") {
    if (step !== "gender") { await sendMessage(chatId, T[lang].welcome, startKeyboard(lang)); return; }
  ...
  if (cb?.data === "confirm_yes") {
    ...
    try {
      const { sendRegistrationEmail } = await import("@/lib/registration-email.server");
      await sendRegistrationEmail({ registrationId: inserted.registration_id, fullName: answers.full_name!, christianName: answers.christian_name!, gender: answers.gender!, birthDateEc: date?.formatted ?? answers.birth_date_ec!, motherName: answers.mother_name!, motherPhone: answers.mother_phone!, fatherName: answers.father_name!, fatherPhone: answers.father_phone!, createdAt: inserted.created_at });
    } catch { console.error("Registration confirmation email could not be sent"); }
    return;
  }
  ...
  if (step === "birth_date_ec") {
    ...
    if (!date) { await sendMessage(chatId, T[lang].errDate(currentEthiopianYear())); return; }
    value = date.formatted;
  } else if (step === "mother_phone" || step === "father_phone") {
    value = validatePhone(text);
    if (value === null) { await sendMessage(chatId, T[lang].errPhone); return; }
  } else if (step === "christian_name") {
    value = validateAmharicName(text, null, lang);
    if (value === null) { await sendMessage(chatId, T[lang].errChristianName); return; }
  } else if (step === "mother_name" || step === "father_name") {
    // Parent names include የአያት and are NOT limited to exactly 3 words.
    value = validateAmharicName(text, null, lang);
    if (value === null) { await sendMessage(chatId, T[lang].errName); return; }
  } else {
    // Q1 remains exactly 3 words.

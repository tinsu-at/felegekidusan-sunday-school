...
export const exportRegistrationsCsvV2 = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const csvRows = rows.map((row) => {
      ...
      const values = [row.registration_id, answerValue(row, { field_key: "full_name" } as QuestionConfig), answerValue(row, { field_key: "christian_name" } as QuestionConfig), answerValue(row, { field_key: "gender" } as QuestionConfig), answerValue(row, { field_key: "birth_date_ec" } as QuestionConfig), ...remainingColumns.map(({ question: columnQuestion }) => { const q = questionByKey.get(columnQuestion.field_key); if (!q || !applicable(q, ageGroup)) return ""; const value = answerValue(row, q); return value || (q.required ? "" : "-"); }), ageGroup, row.age_years, rowVersion, row.status, row.created_at];
